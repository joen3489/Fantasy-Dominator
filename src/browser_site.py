  ...
  <script>
    ...
    function managerTradeFitMarkup(dossier) {{
      ...
      const fitCards = tradeFits.length
        ? `<div class="brief-list">${{tradeFits.map((row, index) => briefCard({{ title: row.player_name || 'Unknown player', category: categoryFor('edge_type', row.edge_type), rank: index + 1, chips: [row.position, row.edge_type, row.trade_edge_score ? `edge ${{row.trade_edge_score}}` : '', row.confidence ? `confidence ${{row.confidence}}` : ''], summary: row.risk || 'Conversation hypothesis; verify the price.', evidence: row.evidence || 'No evidence supplied.', decisionKey: `manager-fit:${{String(dossier.roster_id || 'unknown')}}:${{String(row.player_id || index)}}`, decisionType: 'manager_fit', decisionSubjectId: row.player_id || '', decisionSubjectName: row.player_name || 'manager fit', decisionConfidence: row.confidence || '', decisionRisk: row.risk || '', decisionEvidence: row.evidence || '' }})).join('')}}</div>`
