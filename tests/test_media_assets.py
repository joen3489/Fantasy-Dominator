...
class MediaAssetContractTests(unittest.TestCase):
    ...
    def test_generated_browser_javascript_parses_as_javascript(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            section_art = [item for item in manifest["assets"] if item["asset_type"] == "section_art"]
            self.assertEqual({item["article_key"] for item in section_art}, {"daily_brief", "team_report", "market_watch", "trade_desk"})
            self.assertTrue(all((site / item["path"]).is_file() for item in section_art))
            self.assertTrue(all(item["scope"] == "article" and item["reporter_id"] for item in section_art))
            ...
            self.assertIn("Reality Check packet", html)
            self.assertIn("renderRealityCheckReceipt", html)
            self.assertIn("reporterLensMarkup", html)
            self.assertIn("reporterLensReceipt", html)
            self.assertIn("reporterKey: 'trade_desk'", html)
            self.assertIn("reporterKey: 'market_watch'", html)
            self.assertIn("reporterKey: 'team_report'", html)
            self.assertIn("reporterKey: 'manager_intel'", html)
            self.assertIn("reporterLensReceipt('manager_intel')", html)
            self.assertIn('id="active-manager-lens"', html)
            self.assertIn("renderEditorialMedia", html)
            self.assertIn("currentLlmCount", html)
            self.assertIn("evidence-led fallback", html)
            self.assertIn("held for review", html)
            self.assertIn("publicationMediaMarkup", html)
            self.assertIn("Desk atmosphere", html)
            self.assertIn("data-media-asset", html)
